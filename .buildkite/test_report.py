...
class ReportTests(unittest.TestCase):
    ...
    def test_failure_and_unrun_checks_are_not_green(self):
        states = dict.fromkeys(STAGES, 'waiting_failed')
        states.update(prepare='passed', lint='failed')
        markdown, style = render(states)
        self.assertEqual(style, 'error')
        self.assertIn('1/11 checks passed', markdown)
        self.assertIn('| Lint | failed |', markdown)
        self.assertIn('| Build wheel | waiting_failed |', markdown)
